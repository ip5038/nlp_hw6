...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, hidden, num_heads, fcn_hidden, dropout=0.0):
        ...
        self.self_attention = MultiHeadAttention(
            ...
            hidden=hidden,
            num_heads=num_heads,
            causal=True,  # causal masking for decoder self-attention
        )
    ...
    def forward(self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None):
        ...
        # 1-3: Self-attention -> residual -> LayerNorm
        residual = decoder_hidden_states                                    # [batch_size, tgt_len, hidden]
        x = self.self_attention(decoder_hidden_states)                      # [batch_size, tgt_len, hidden]
        x = self.att_layer_norm(x + residual)                              # [batch_size, tgt_len, hidden]
    
        # 4-6: Cross-attention -> residual -> LayerNorm
        residual = x                                                        # [batch_size, tgt_len, hidden]
        x = self.cross_attention(x, kv=encoder_hidden_states,              # [batch_size, tgt_len, hidden]
                                 key_padding_mask=key_padding_mask)
        x = self.cross_att_layer_norm(x + residual)                        # [batch_size, tgt_len, hidden]
    
        # 7-10: FCN -> dropout -> residual -> LayerNorm
        residual = x                                                        # [batch_size, tgt_len, hidden]
        x = self.fcn(x)                                                    # [batch_size, tgt_len, hidden]
        x = self.dropout(x)                                                # [batch_size, tgt_len, hidden]
        x = self.fcn_layer_norm(x + residual)                              # [batch_size, tgt_len, hidden]
...
class TransfomerEncoderDecoderModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encoder_layers = nn.ModuleList([
            TransformerEncoderLayer(hidden=hidden, num_heads=num_heads, fcn_hidden=fcn_hidden, dropout=dropout)
            for _ in range(num_layers)
        ])
        self.decoder_layers = nn.ModuleList([
            TransformerDecoderLayer(hidden=hidden, num_heads=num_heads, fcn_hidden=fcn_hidden, dropout=dropout)
            for _ in range(num_layers)
        ])
        # YOUR CODE ENDS HERE
    ...
    def _encode(self, input_ids, key_padding_mask):
        ...
        x = self.encoder_embeddings(input_ids)          # [batch_size, src_len, hidden]
        x = self._add_positions(x)                      # [batch_size, src_len, hidden]
        x = self.dropout(x)                             # [batch_size, src_len, hidden]
    
        for layer in self.encoder_layers:
            x = layer(x, key_padding_mask=key_padding_mask)  # [batch_size, src_len, hidden]
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        # YOUR CODE STARTS HERE
        x = self.decoder_embeddings(decoder_input_ids)  # [batch_size, tgt_len, hidden]
        x = self._add_positions(x)                      # [batch_size, tgt_len, hidden]
        x = self.dropout(x)                             # [batch_size, tgt_len, hidden]
    
        for layer in self.decoder_layers:
            x = layer(
                decoder_hidden_states=x,
                encoder_hidden_states=encoder_hidden_states,
                key_padding_mask=key_padding_mask,
            )                                           # [batch_size, tgt_len, hidden]
    
        logits = self.out_proj(x)                       # [batch_size, tgt_len, tgt_vocab_size]
