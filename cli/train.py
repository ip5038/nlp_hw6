...
def parse_args():
    ...
    # Data arguments
    parser.add_argument("--dataset_name", type=str, default="stas/wmt14-en-de-pre-processed",
        help="The name of the dataset to use (via the datasets library).",
    )
    parser.add_argument(
        "--dataset_config_name", type=str, default="ende",
    ...
    parser.add_argument("--source_lang", type=str, required=True, help="Source language id for translation.")
    parser.add_argument("--target_lang", type=str, required=True, help="Target language id for translation.")

    parser.add_argument("--debug", default=False, action="store_true",
        help="Whether to use a small subset of the dataset for training (useful for debugging).",
    )

    # Model arguments
    parser.add_argument("--num_layers", default=6, type=int, help="Number of hidden layers in the Transformer encoder")
    parser.add_argument("--hidden_size", default=512, type=int, help="Hidden size of the Transformer encoder")
    parser.add_argument("--num_heads", default=8, type=int, help="Number of attention heads in the Transformer encoder")
    parser.add_argument("--fcn_hidden", default=2048, type=int, help="Hidden size of the FCN")
    parser.add_argument("--max_seq_length", type=int, default=128,
        help="The maximum total sequence length for source and target texts after tokenization. "
    ...
    parser.add_argument("--learning_rate", type=float, required=True, help="Initial learning rate to use. Good value is 5e-5")
    parser.add_argument("--weight_decay", type=float, default=0.0, help="L2 regularization strength.")
    parser.add_argument("--dropout_rate", type=float, default=0.1, help="Dropout rate of the Transformer encoder")
    parser.add_argument("--num_train_epochs", type=int, default=1, help="Total number of training epochs to perform.")
    parser.add_argument("--eval_every_steps", type=int, default=5000, help="Perform evaluation every n network updates.")
    parser.add_argument("--logging_steps", type=int, default=10, help="Compute and log training batch metrics every n steps.")
    parser.add_argument("--lr_scheduler_type", type=transformers.SchedulerType, default="linear",
        help="Learning rate scheduler type to use.",
        choices=["linear", "cosine", "cosine_with_restarts", "polynomial", "constant", "constant_with_warmup"],
    )
    parser.add_argument("--num_warmup_steps", type=int, required=True, help="Number of steps for the warmup in the lr scheduler.")

    # Evaluation arguments
    parser.add_argument("--generation_type", choices=["greedy", "beam_search"], default="beam_search")
    parser.add_argument("--beam_size", type=int, default=5,
        help=("Beam size for beam search generation. Decreasing this parameter will make evaluation faster, "
    ...
    parser.add_argument("--seed", type=int, default=None, help="A random generator seed for reproducible training.")
...
def preprocess_function(
    ...
    #
    # The loop creates two sequences from each target decoder_input_ids and labels.
    #
    # END OF YOUR ANSWER
    decoder_input_ids = []
    labels = []
    ...
    #
    # We shift so the decoder's input is previous token and label is the current token (the correct answer).
    #
    # END OF YOUR ANSWER
...
def evaluate_model(
    model,
    dataloader,
    *,  # this is what this means: https://stackoverflow.com/questions/53797057/star-as-an-argument-in-python-function
    target_tokenizer,
    device,
    max_seq_length,
    generation_type,
    ...
    for batch in tqdm(dataloader, desc="Evaluation"):
        with torch.inference_mode():
            ...
            # YOUR ANSWER HERE (please limit your answer to 1-2 sentences):
            #
            # Model forward does a single pass through the model
            # Model generate produces one token at a time until EOS or max lenght is reached.
...
def main():
    ...
    wandb.init(project="transformer_mt", config=args)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            if global_step % args.eval_every_steps == 0 or global_step == max_train_steps:
