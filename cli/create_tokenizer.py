...
def main():
    ...
    # Task 3.1 (2 points): by analogy to the source tokenizer above, make a tokenizer for the target language
    ...
    # Build a BPE tokenizer for the target language
    target_tokenizer = Tokenizer(BPE(unk_token="[UNK]"))
    
    # Create a trainer with special tokens and vocab size
    ...
    # Create an iterator over the target language sentences
    target_iterator = (item["translation"][args.target_lang] for item in raw_datasets["train"])
    
    # Train the tokenizer on the target language data
    target_tokenizer.train_from_iterator(
        target_iterator,
        trainer=target_tokenizer_trainer,
    )
    
    # Wrap it in HuggingFace's PreTrainedTokenizerFast
